...
const REVIEW_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      practice: {
        type: "array",
        description: "2–4 Übungspakete für zuhause, jeweils zu einer Schwachstelle. An die/den SchülerIn formuliert.",
        items: {
          type: "object",
          properties: {
            title: { type: "string", description: "Kurzer Titel, z. B. 'Kehrwert bei gemischten Zahlen'." },
            description: { type: "string", description: "Was genau zu tun ist, 1–2 Sätze, Du-Form." },
            weakness: { type: "string", description: "Die Schwachstelle aus der Liste oben, an der dieses Paket arbeitet." },
            resource_id: { type: "string", description: "id eines Materials aus <material>, das wirklich zu dieser Übung passt. Sonst leerer String. Niemals eine id erfinden oder einen Link ausdenken." },
            exercises: {
              type: "array",
              description: "3–6 konkrete Aufgaben, die ohne weiteres Material lösbar sind, aufsteigend im Schwierigkeitsgrad und passend zur Klassenstufe.",
              items: {
                type: "object",
                properties: {
                  question: { type: "string", description: "Die Aufgabe selbst, vollständig formuliert. Mathematik in gut lesbarer Schreibweise, z. B. 3/4 : 1/2." },
                  hint: { type: "string", description: "Ein kurzer Tipp, der den Weg zeigt, ohne die Lösung zu verraten." },
                  solution: { type: "string", description: "Die Lösung mit kurzem Rechenweg bzw. kurzer Begründung." },
                },
                required: ["question", "hint", "solution"],
              },
            },
          },
          required: ["title", "description", "weakness", "resource_id", "exercises"],
        },
      },
...
- Übungen: Jedes Übungspaket gehört zu genau einer Schwachstelle und enthält eigene, fertig formulierte Aufgaben mit Tipp und Lösung. Rechne jede Lösung sorgfältig nach – lieber eine Aufgabe weniger als eine falsche.
- Material: Du bekommst die Material-Bibliothek der Lehrerin. Verknüpfe ein Material nur, wenn es inhaltlich wirklich passt, und nutze dafür ausschließlich die dort genannten ids. Erfinde niemals Links oder ids.
...
Deno.serve(async (req) => {
  ...
  const [{ data: st }, { data: prev }, { data: hw }, { data: res }] = await Promise.all([
    ...
    sb.from("resources").select("id, title, kind, subject, note, student_ids").limit(200),
  ]);
  // Material, das diese/r SchülerIn sehen darf
  const material = (res ?? []).filter((r: any) => !r.student_ids?.length || r.student_ids.includes(lesson.student_id))
    .map((r: any) => `- id: ${r.id} | ${r.kind} | ${r.subject} | ${r.title}${r.note ? " (" + r.note + ")" : ""}`).join("\n") || "keine";
...
Offene Hausaufgaben: ${(hw ?? []).map((h: any) => `${h.title} (${h.subject})`).join("; ") || "keine"}`;
  ...
  const r = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `<kontext>\n${context}\n</kontext>\n\n<material>\n${material}\n</material>\n\n<transkript>\n${lesson.transcript.slice(0, 180000)}\n</transkript>\n\nWerte diese Stunde aus und baue die Übungspakete.`,
      }],
    }),
  });
  if (!r.ok) {
  ...
  const update: Record<string, unknown> = {
    ...
    practice: (rv.practice ?? []).map((p: any) => ({
      title: p.title,
      description: p.description,
      weakness: p.weakness ?? "",
      // nur ids übernehmen, die es wirklich gibt
      resource_id: (res ?? []).some((r: any) => r.id === p.resource_id) ? p.resource_id : "",
      exercises: (p.exercises ?? []).slice(0, 8).map((e: any) => ({ question: e.question, hint: e.hint ?? "", solution: e.solution ?? "" })),
    })),
    review: {
